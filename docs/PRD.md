...
v2.3 정의 유지. `hyperparameters.yaml`·`providers.yaml`·`prompts/`·`glossary/`는 `config/agents.yaml`과 같은 위치의 **미배선 스펙**(라우터·생성 경로가 아직 로드 안 함; prompts는 코드 하드코딩). 후속 배선 대상.
...
4. **S10 후속 후보**: `config/agents.yaml` hot-reload(PRD §7.5.5) · hyperparameters/providers/prompts/glossary 배선 · Langfuse 추적(S15) · embedding/reranker/tts 섹션의 라우터 연결.
