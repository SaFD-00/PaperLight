...
### v3.1
1. **§7.5 라우팅 전면 Qwen3.6화** — 전 agent를 OpenRouter Qwen3.6 패밀리(`plus`/`flash`/`35b-a3b`)로 통일(gpt-5·gemini 제거, vision 포함) + agent별 `reasoning_effort`(`none~high`) 도입·실배선. 라우팅 설정 파일을 `config/models.yaml` → 레포 루트 `config/agents.yaml`로 일원화(`tasks:`→`agents:`).
...
8. **§7.5 default 모델** — `openrouter / qwen/qwen3.6-35b-a3b` ⭐ 실험·실행 기본. 전 agent Qwen3.6 패밀리 + agent별 `reasoning_effort`.
...
- **본문만 번역**: Figure 캡션·표·수식·페이지번호·이메일·arXiv 식별자(세로 스탬프)·1페이지 front-matter(저자·소속·Project/Dataset/Model 링크; 제목만 유지) 등 비본문을 보수적으로 제거(노이즈 제거).
...
7. **전체 논문 맥락 주입** ⭐(v3.1): per-element 태스크(F-04 설명, F-14 도표, F-15 단락, F-03 챗, F-02 번역)가 로컬 텍스트만 보던 한계를 보완해, **(a) pregen `summary` 캐시 + (c) RAG 관련 청크**를 "[논문 맥락]" 블록으로 프롬프트에 주입(`agents/context.py`). 지어내기 방지 가드 동봉. **번역은 용어 일관성용 요약만(RAG 제외)** 주입하고 정렬 번역의 **문장 1:1 대응 보존**(요약은 system에만, numbered user 입력 불변). 맥락은 `(paper_id, text)`의 결정적 함수라 캐시 키 정합성 유지 → prompt_version bump로만 무효화(explain v2 / figure·table v3 / paragraph v2 / chat v2 / translate v2).
...
#### 7.5.4 `config/agents.yaml` ⭐ 전 agent = OpenRouter Qwen3.6 패밀리 + `reasoning_effort`
라우터가 로드하는 단일 소스(레포 루트 `config/agents.yaml`). 모든 LLM agent를 OpenRouter Qwen3.6 패밀리(`plus`/`flash`/`35b-a3b`)로 라우팅하고, agent별 `reasoning_effort`(`none|low|medium|high`)를 지정한다(운영 기조: 대부분 `none~medium`, 팟캐스트 창작만 `high`). `reasoning_effort`는 `router`가 읽어 OpenRouter 요청의 `reasoning.effort`로 전달한다. vision(figure/table)도 Qwen3.6-plus 멀티모달로 처리. embedding/reranker/tts는 라우터 미사용 보존 섹션.
...
agents:
  ...
  chat:        # primary 35b-a3b, fallback plus
    provider: openrouter
    model: qwen/qwen3.6-35b-a3b
    reasoning_effort: medium
  ...
  figure_description:        # F-14 — vision (qwen3.6-plus)
    provider: openrouter
  ...
  # explanation·paragraph_description·podcast_outline·podcast_critique·
  # critique·classifier·highlight·domain_classifier 동일 패턴(전 15 agent)
  # — 전문은 config/agents.yaml 참조.
...
> **마지막 갱신**: 2026-06-03 (Phase 2 4대 제품 기능 stub-first 구현 — F-13/F-09/F-07/Export-Notion)
> **참조**: [ROADMAP.md](./ROADMAP.md) — Phase별 task 분해
> **현재 마일스톤**: → **Phase 2** — F-13 Podcast ✅ stub-first(키 없으면 placeholder mp3) · F-09 Deep Search ✅ stub-first · F-07 Preview ✅ · Export-Notion ✅ stub-first. (Phase 1 MVP S7~S16 코드 완료.) i18n·Observability 안정화·Payment·라이브 키/배포 대기
...
| Phase 1 (8주) | 🚧 진행 | S7a ✅ · S7b 🚧 (Auth Stub/Mock) · S8 ✅ (arXiv import) · S9 ✅ (Ingestion: PyMuPDF→chunk→embed→Qdrant) · S10 ✅ (LLM Abstraction: config/agents.yaml 라우팅 + fallback + 응답 캐시 + reasoning_effort) · S11 ✅ (Auto pre-gen: summary/highlight/F-14/F-15 → 영구 캐시 + Right Panel Summary·Insights 탭) · S12 ✅ (Chat+Citation: RAG SSE 채팅 + 대화 영속화 + 인용 페이지 점프 + References 패널) · S13 ✅ (Library 4-pane: `/api/library` 컬렉션 트리·태그·상태·Bulk·본문 검색·BibTeX/RIS/EndNote import-export + FE 4-pane) · S14 ✅ (Markup: `/api/annotations` 하이라이트 CRUD·노트 upsert+S3 백업·Markdown/Obsidian export + pdf.js overlay 채널 3개 + Floating Menu 5액션 + NotesPanel /ai) · S15 ✅ (Observability: Sentry FE+BE 에러 + Langfuse `stream_task` LLM 트레이스 + PostHog FE 큐레이션 이벤트, 전부 env 게이팅·키 부재 시 no-op) · S16 ✅ (CI: GitHub Actions — backend ruff/mypy/pytest 182, frontend typecheck/lint/vitest 31/`next build`, Playwright e2e 30 별도 job·BE는 `page.route` 목킹이라 불필요) · 실 논문 PDF 연결(Center→`/api/papers/{id}/pdf-url` presigned). 다음 **Phase 2**. [ROADMAP §3](./ROADMAP.md) |
| Phase 2 (12주) | 🚧 진행 | **4대 제품 기능 stub-first 구현**: F-13 Podcast(`podcast_graph`+TTS stub-first+stitch/SRT+🎧 패널) · F-09 Deep Search(관심 벡터+추천+'왜 추천'+`/discover`) · F-07 Preview(cross-ref 호버 도표 크롭) · Export-Notion(미연동 시 마크다운 fallback). 외부 키·OAuth 없이 오프라인/CI green, 마이그레이션 0. 회귀: BE 202·vitest 36·e2e 34·build. i18n·Observability·Payment·라이브 키 대기 — [ROADMAP §4](./ROADMAP.md) |
...
| F# | 이름 | Phase | 상태 | 비고 |
|----|------|-------|------|------|
| F-01 | PDF 뷰어 (pdf.js + Shadow DOM) | 0 | ✅ (Phase 0 범위) | S2 — pdf.js 4.10 + Shadow DOM iframe + 7-채널 postMessage (LOAD/JUMP/ZOOM/HIGHLIGHT/TOGGLE_TRANSLATION/REQUEST_PAGE_TEXT/PAGE_TEXT). Phase 1에서 arXiv import 합류 시 보강. **페이지 내 검색(§5.6.6)**: FIND/FIND_STEP/FIND_CLEAR↔FIND_RESULT 채널 + text-layer 부분일치 오버레이(현재 일치 주황 강조·% 앵커로 줌 무관) + SearchBar(⌘F·툴바 🔍, n/N 카운터·이전/다음·Esc). |
| F-02 | Translation (페이지별 컬럼) | 0 | ✅ (Phase 0 범위) | S5 — `/api/translate` SSE(aligned) + TopToolbar [T] 토글. **각 페이지 옆 번역 컬럼**(viewer.js `.page-row`, lazy 스트리밍·페이지별 캐시), **본문만 파싱**(bodyFilter.js: Figure/표/수식/페이지번호·이메일·arXiv 식별자·1페이지 front-matter[저자·소속·링크, 제목만 유지] 제거, offset 보존 segments), iframe 내부 양방향 교차 하이라이트, **글꼴(세리프/산세리프)·리더 글꼴 크기** 설정(persist). |
| F-03 | AI Chat | 1 | ✅ | S12 — `POST /api/chat` RAG SSE(`agents/chat.py` 질문 임베딩→Qdrant 검색→grounded 답변) + `{citations}`/`{followups}` 이벤트 + 대화 영속화(`ChatSession`/`ChatMessage`)·멀티턴 + `GET /api/chat/{id}` 히스토리 + ChatPanel(정적 퀵칩 + 후속칩 + 인용 칩→페이지 점프). 응답 캐시 `chat-v2` TTL 30일. **전체 논문 맥락**: RAG top-4 외에 `summary` 캐시를 system에 주입(`agents/context.py`) — 근거는 발췌문, 요약은 흐름 파악용 |
| F-04 | Explanation | 0 | ✅ (Phase 0 범위) | S4 — Floating Menu + `/api/explain` OpenRouter SSE relay + ExplanationPanel 스트리밍. + `/api/explain/figure`(F-14 인라인 비전 설명, crop 이미지+텍스트). **전체 논문 맥락**: 선택 단락에 요약+RAG 주입(`explain-v2`, `agents/context.py`), 도표는 첫 턴에 주입(`figure-v3`/`table-v3`) |
...
| F-07 | Preview | 2 | ✅ | Phase 2 — 본문 cross-ref(Figure N/Table N/그림·표) 호버 → 도표 영역 PNG 크롭 팝오버(`GET /api/papers/{id}/figures/{idx}/image` = `render_region` 크롭, layout 없으면 텍스트만). FE `lib/crossref`(순수 토큰 분리·label 매칭)+`CrossRefText`+Markdown `crossRefPaperId` opt-in→SummaryPanel. 정밀 bbox는 marker-pdf 후속 |
...
| F-09 | Scholar Deep Search | 2 | ✅ | Phase 2 — 라이브러리 청크 임베딩 평균(`library_service.interest_vector`, 정규화) → 추천 + '왜 추천' 설명(`POST /api/deep-search`, `GET /api/deep-search/interests`). stub-first(`DEEP_SEARCH_PROVIDER` 미설정 시 쿼리 해시 결정적 카드, `semantic_scholar` opt-in→stub 폴백). FE `/discover` 검색+카드+'왜 추천' 토글, ⌘K 진입. Cohere 재랭킹은 후속 |
| F-10 | Auto Highlight | 1 | ⬜ | Ingestion 자동 |
| F-11 | Markup (노트+하이라이트) | 1 | ✅ | Phase 1 (S14) + Phase 2 Notion export stub-first(`POST /api/annotations/papers/{pid}/export/notion`, `NOTION_TOKEN`+`NOTION_PARENT_PAGE_ID` 미설정 시 마크다운 클립보드 fallback / 설정 시 `pages.create` best-effort). 기존 Obsidian/Markdown 유지. BibTeX/RIS/EndNote는 Phase 1 S13 |
| F-12 | Team / 협업 | 3 | ⬜ | Phase 3 |
| F-13 | AI Podcast ⭐ | 2 | ✅ | Phase 2 차별 포인트 — 논문→2인 대담(`podcast_graph.build_script`/`parse_script`) → 세그먼트별 TTS(`tts_openai` stub-first: 키 없으면 결정적 placeholder mp3, 있으면 tts-1-hd) → `stitch`/SRT → 오디오 HMAC 가드 라우트. `POST /api/podcast` + BackgroundTasks 잡, status SSE, 🎧 PodcastPanel(생성→폴링→`<audio>`+대본). ElevenLabs·재생 중 동기 하이라이트는 후속 |
| F-14 | Figure/Table Description | 1 | 🚧 | Ingestion 자동(pregen 텍스트 reasoning) + **인라인 비전 설명**: 리더가 캡션 감지→도표 crop 이미지+텍스트를 비전 모델로 `/api/explain/figure` 스트리밍(그림/표 분석 팝오버). 프로바이더 멀티모달 content(`providers/content.py`). 정밀 bbox는 marker-pdf 후속. pregen·인라인 모두 **전체 논문 맥락**(요약+RAG) 주입(`figure-v3`/`table-v3`) |
| F-15 | Paragraph-level Description | 1 | ⬜ | Ingestion 자동. **전체 논문 맥락**: description=요약+RAG(자기 제외), importance=요약만(한 단어 분류 유지) — `paragraph-desc-v2`/`paragraph-importance-v2` |
...
| Auth (Google OAuth + JWT + Cookie) | ✅ | S7b — `python-jose` HS256 + httpOnly cookie (access 15m / refresh 30d, `Path=/api/auth/refresh`) + refresh rotation + reuse detection + `/api/auth/dev/mock-login` + **Google OAuth 2.0 authorization-code flow**(`auth/google.py`: authUrl + callback, CSRF state 쿠키, userinfo sub·email upsert). `GOOGLE_OAUTH_CLIENT_ID/SECRET` 설정 시 활성, 미설정 시 mock 폴백 |
...
| LLM Abstraction (S10) | ✅ | `config/agents.yaml`(default + 15 agent, 전 agent Qwen3.6 + `reasoning_effort`) + `providers/router.py`(`candidates`/`primary_model`/`reasoning_effort`/`stream_task` fallback) + Provider 레지스트리(OpenRouter 실 + 실 OpenAI/Gemini graceful + `LLM_PROVIDER=stub` 오프라인) + `providers/cache.py` 응답 캐시(Cache ORM, 키 `sha256(task+paper_id+chunk_id+model+prompt_version)`). explain/translate 라우터+캐시 경유. Langfuse(S15)·hot-reload는 후속 |
...
### 18.4 결정 완료 사항 (변경 없음)
- 기본 LLM: `openrouter / qwen/qwen3.6-35b-a3b` ([§7.5](#75-llm-라우팅)) — 전 agent Qwen3.6 패밀리 + agent별 `reasoning_effort`
...
### 18.5 다음 액션
1. **다음 진입점**: **Phase 2 잔여** — i18n(en/ja/zh-CN/es) · Observability 안정화(Prometheus/Grafana/OpenTelemetry) · GDPR ZIP export · 결제 검토 + 4대 기능의 라이브 키 후속(ElevenLabs·Cohere rerank·Notion OAuth·marker-pdf 정밀 bbox·동기 하이라이트). **4대 제품 기능(F-13/F-09/F-07/Export-Notion)은 stub-first로 end-to-end 구현 완료** — 외부 키 없이 오프라인/CI green, 마이그레이션 0. 회귀: BE pytest 202·vitest 36·Playwright e2e 34(s18~s21 신규)·`next build`.
2. **병행 보류**: **Google OAuth 라이브** — 코드 완료(`/api/auth/login/google`→`/google/callback`→세션 발급→`/library` 리다이렉트, 미구성 시 503 graceful). `GOOGLE_OAUTH_CLIENT_ID/SECRET` 발급 시 즉시 동작. dev mock-login 유지.
3. **S8/S9 후속 후보**: marker-pdf 파서(`INGEST_PARSER=marker`) 실구현 · fastembed bge-m3 실 임베딩 검증 · reader `/r/{id}`에 presigned PDF 실렌더 연결 ✅(S16, Center→`/pdf-url`) · Celery+Redis 전환 · Cohere rerank(Phase 2).
4. **S10 후속 후보**: `config/agents.yaml` hot-reload(PRD §7.5.5) · prompts/glossary 배선 · Langfuse 추적(S15) · embedding/reranker/tts 섹션의 라우터 연결. (hyperparameters·providers는 §7.5.5대로 배선·제거 완료.)
...
14. S16 검증: BE pytest 182/182 PASS (test_router 5건 동기화 — c2a6b9e의 mid=flash tier 재배정에 기대값 일치) + frontend typecheck clean·vitest 31/31·`next build` clean + Playwright e2e 30/30 PASS (Phase 0~S17 회귀, 신규 spec 0) + CI(`.github/workflows/ci.yml`) backend/frontend/**e2e** 3-job(frontend에 vitest·build 스텝 추가, Playwright 별도 job — BE는 `page.route` 목킹이라 불필요). 실 논문 PDF 연결: `Center.tsx`가 sample 외 paperId에 `/api/papers/{id}/pdf-url` presigned 비동기 조회. **Phase 1 MVP 코드 완료** — 잔여는 라이브 배포(Vercel/Render)·OAuth 자격증명뿐.
