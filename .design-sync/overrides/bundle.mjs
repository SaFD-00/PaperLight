// forked from design-sync lib/bundle.mjs — PaperLight is a Next.js app: bundled
// app code (lib/api.ts) reads process.env.NEXT_PUBLIC_* and bundled remark/vfile
// polyfills reference bare `process`, which is undefined in the browser and threw
// `ReferenceError: process is not defined` on every preview (and every design).
// Adds a `process` shim banner to the IIFE so the real code runs in a browser.
// The banner sits below stampHeader's `/* @ds-bundle */` line and above the
// window.<GLOBAL>= footer — the output contract is unchanged.
//
